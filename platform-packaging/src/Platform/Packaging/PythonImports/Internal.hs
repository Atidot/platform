...
module Platform.Packaging.PythonImports.Internal where
...
getExplicitPkgOrigins fileContents = do
    ...
    fmap (map (\(dn, pkg) -> (dottedToModuleName dn, pkg))) $ mapM processPkg explicitPkgs
    where
        processPkg :: Token -> Maybe (DottedName SrcSpan, PyPkg)
        processPkg tkn = do
            importedMod <- fmap ( getForeignImportNames
                         . Module
                         . return)
                         $ stmtBeforeComment tkn
            fmap (\dn -> (dn, pkgFromToken tkn))
                 $ headIfLengthIsOne' importedMod
        headIfLengthIsOne' :: [a] -> Maybe a
        headIfLengthIsOne' []     = Nothing
        headIfLengthIsOne' (x:xs) = Just x
        comesAfterImport :: Token -> Maybe Bool
        comesAfterImport = return . isImport . stmtBeforeComment
        hasPkgAnnotation :: Token -> Bool
        hasPkgAnnotation tkn
          = (token_literal tkn) =~ explicitPkgRegex
         || (token_literal tkn) =~ suppressPkgRegex
        stmtBeforeComment :: Token -> Maybe (Statement SrcSpan)
        stmtBeforeComment tkn = do
            file <- fileFromCommentLine tkn
            case parseStmt file "" of
              Right (stmt : stmts, _) -> Just stmt
              _                       -> Nothing
        fileFromCommentLine :: Token -> Maybe String
...
readInit fp = do
    initAST <- maybe (throwM FileNotParseable) return (getAST fp)
